...
public class MeisheSdkXFWXModule extends WXSDKEngine.DestroyableModule   {
    ...
    @JSMethod(uiThread = true)
    public void setPreTc() {
        setPreTc2();
    }
    @JSMethod(uiThread = true)
    public void setNextTc() {
        setNextTc2();
    }
    @JSMethod(uiThread = true)
    ...
    private void startScroll2(){
        ...
        if (type!=0){
            if (type==1){
                ...
                Toast.makeText(Mainactivity,"当前处于启动状态",Toast.LENGTH_LONG).show();
            }
        }else {
            Toast.makeText(Mainactivity,"悬浮框未打开，请先打开",Toast.LENGTH_LONG).show();
        }
    }
     ...
     *
     * */
    private void stopScroll2(){
        int type = getScrollStatus();
        if (type!=0){
            if (type==1){
                Toast.makeText(Mainactivity,"当前处于停止状态",Toast.LENGTH_LONG).show();
            }else {
                if (VarManger.isXfkLandSpaceShow){
                    PopupManager.getInstance().beyondViewLandSpace.changeTvStatus();
                }else {
