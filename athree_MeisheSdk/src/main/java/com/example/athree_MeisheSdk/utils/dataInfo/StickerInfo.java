package com.example.athree_MeisheSdk.utils.dataInfo;
...
public class StickerInfo {
     ...
     * 关键帧
     */
    private Map<Long, StickerKeyFrameInfo> mKeyFrameInfoHashMap = new TreeMap<>(
            new Comparator<Long>() {
                @Override
    ...
    public void putKeyFrameInfo(long keyFrameTime, StickerKeyFrameInfo stickerKeyFrameInfo) {
        if (mKeyFrameInfoHashMap != null) {
            mKeyFrameInfoHashMap.put(keyFrameTime, stickerKeyFrameInfo);
        }
    }

    public Map<Long, StickerKeyFrameInfo> getKeyFrameInfoHashMap() {
    ...
    public static class StickerKeyFrameInfo {
        float mKeyFrameScaleFactor;
        float mKeyFrameRotation;
        PointF mKeyFrameTranslation;

        public float getKeyFrameScaleFactor() {
            return mKeyFrameScaleFactor;
        }

        public float getKeyFrameRotation() {
            return mKeyFrameRotation;
        }

        public PointF getKeyFrameTranslation() {
            return mKeyFrameTranslation;
        }

        public StickerKeyFrameInfo(float keyFrameScaleFactor, float keyFrameRotation, PointF keyFrameTranslation) {
            this.mKeyFrameScaleFactor = keyFrameScaleFactor;
            this.mKeyFrameRotation = keyFrameRotation;
            this.mKeyFrameTranslation = keyFrameTranslation;
        }
    }
}
