...
public class TcViewOtherDirection extends LinearLayout {

    public TcViewOtherDirection(Context context) {
        this(context, null);
    }

    public TcViewOtherDirection(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TcViewOtherDirection(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void initView() {
        ...
        mView = LayoutInflater.from(context).inflate(R.layout.dialog_app_text_other_direction, this);
...
//                }
                viewEvent.changeScreenOri(false,false);
            }
        });
...
//                            }
    ...
    public boolean onTouchEvent(MotionEvent it) {
        switch (it.getAction()) {
            case MotionEvent.ACTION_DOWN:
                isRightBottom = isRightTop(this, (int) it.getX(), (int) it.getY());
                lastRawX = (int) it.getRawX();
                lastRawY = (int) it.getRawY();
...
//        tvContent.scrollToY(xfkHei);
    ...
    public Boolean isRightTop(View v, int x, int y) {
        if (v.getRight() - v.getLeft() - x < VarManger.dip2px(context, 35f) && y - v.getTop()< VarManger.dip2px(context, 35f)) {
            return true;
        }
        return false;
    }
